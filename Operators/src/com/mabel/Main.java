package com.mabel;

public class Main {

    public static void main(String[] args) {
	// write your code here
        int result = 1 + 2;
        System.out.println("1 + 2 = " + result);

        int previousResult = result;

        previousResult = result - 1;
        System.out.println(previousResult + " - 1 = " + result);

        previousResult = result;

        result = result * 10;
        System.out.println(previousResult + " * 10 = " + result);

        previousResult = result;

        result = result / 5;
        System.out.println(previousResult + " /5 10 = " + result);

        previousResult = result;
        result = result % 3;
        System.out.println(previousResult + " % 3 = " + result);

    previousResult = result;
            result = result + 1;
        System.out.println("Resukt is now " + result);
        result++;
        System.out.println("Resukt is now " + result);
        result--;
        System.out.println("Resukt is now " + result);
        result += 2;
        System.out.println("Resukt is now " + result);
        result *= 10;
        System.out.println("Resukt is now " + result);
        result -= 10;
        System.out.println("Resukt is now " + result);
        result /= 10;
        System.out.println("Resukt is now " + result);

        boolean isAlien = false;
        if (isAlien == true)
            System.out.println("It is not alien!");
        int topScore = 80;
        if (topScore < 100)
            System.out.println("You got the higher score");

        int secondTopScore = 95;
        if(topScore > secondTopScore && topScore < 100)
            System.out.println("Greater than top score less than 100");

        if((topScore > 90) || (secondTopScore <= 90))
            System.out.println("One of these test is true");

        int newValue = 50;
        if(newValue == 50)
            System.out.println("This is an true");

        boolean isCar = false;
                if(isCar == true)
                    System.out.println("This is not supposed to happen");

        isCar = true;
        boolean wasCar = isCar ? true : false;
        if(wasCar)
            System.out.println("wasCar is true");

    //exercise
        double a = 20;
        double b = 80;
        double x = (a + b) * 25;
        System.out.println("Mytotal = " + x);
        double totalX = x % 40;
        System.out.println("remainder= " + totalX);
        if(totalX == 20 || totalX < 20)
            System.out.println("Total was over the the limit");
    }
}
